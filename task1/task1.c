...
static void access_register_via_system_instruction(void* junk) {
    ...
    asm volatile("MRS %[temp],MIDR_EL1":[temp]"=r"(reg):); 
    val1[0] = reg;
    asm volatile("MRS %[temp],ID_AA64PFR0_EL1":[temp]"=r"(reg):); 
    val1[1] = reg;
    asm volatile("MRS %[temp],ID_AA64DFR0_EL1":[temp]"=r"(reg):); 
    val1[2] = reg;
    asm volatile("MRS %[temp],ID_AA64ISAR0_EL1":[temp]"=r"(reg):); 
    val1[3] = reg;
    asm volatile("MRS %[temp],ID_AA64MMFR0_EL1":[temp]"=r"(reg):); 
    val1[4] = reg;
...
static int __init task1_init(void) {
    ...
    smp_call_function_single(CPU0, access_register_via_memory_map, cpu0_addr, 1);
    if (!check()) {
    ...
    smp_call_function_single(CPU1, access_register_via_memory_map, cpu1_addr, 1);
    if (!check()) {
    ...
    smp_call_function_single(CPU2, access_register_via_memory_map, cpu2_addr, 1);
    if (!check()) {
        printk(KERN_INFO "CPU2: base address is not correct");
        debug();
    }
    else
    ...
    smp_call_function_single(CPU3, access_register_via_memory_map, cpu3_addr, 1);
    if (!check()) {
